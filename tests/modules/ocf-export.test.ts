...
describe("OCF v1.2.0 export", () => {
  test("generates an officially valid package and preserves all declared losses", () => {
    ...
    const source: OcfExportSource = {
      ...fixture,
      issuer: { ...fixture.issuer, organizationNumber: "5560160680" },
      stakeholders: fixture.stakeholders.map((stakeholder, index) =>
        index === 0 ? { ...stakeholder, taxId: "8507099805" } : stakeholder,
      ),
    };
    ...
    expect(manifest.issuer.tax_ids[0]?.tax_id).toBe("556016-0680");
    expect(stakeholderFile.items[0]?.tax_ids[0]?.tax_id).toBe("850709-9805");
  });
