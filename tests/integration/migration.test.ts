...
describe("database migration transaction", () => {
  ...
  test("normalizes legacy dashed company identifiers", () => {
    ...
    try {
      ...
      insertLegacyCompany(database, "company-dashed", "550000-0004");
      ...
      expect(
        database.sqlite
          ...
          .get(),
      ).toEqual({ registrationValue: "5500000004" });
    } finally {
      database.close();
      rmSync(directory, { recursive: true, force: true });
    }
  ...
  test("rolls back identifier migration when canonical values collide", () => {
    ...
    try {
      migrateDatabase(database, applicationMigrations(directory, 3));
      insertLegacyCompany(database, "company-dashed", "550000-0004");
      insertLegacyCompany(database, "company-plain", "5500000004");
      expect(
        database.sqlite
          .query<{ registrationValue: string }, []>(
          ...
          .all(),
      ).toEqual([{ registrationValue: "550000-0004" }, { registrationValue: "5500000004" }]);
