...
Terminate HTTPS at a reverse proxy and send traffic to container port 3100. The
proxy must preserve the external `Host`/origin semantics, cookies, and `Origin`
header. Do not expose a production HTTP origin directly.
...
cutoffs. Frontend changes must follow `docs/design-system.md` and
`src/web/CLAUDE.md`.
