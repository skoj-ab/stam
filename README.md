...
`dist/server/drizzle`, and the Typst share-register template. The container
includes the pinned Typst compiler, fonts required for PDF exports, and Poppler
utilities required for Fortnox PDF imports.
...
checks `/api/health`, and allows 30 seconds for graceful shutdown. Production
must terminate TLS at a reverse proxy and forward the public origin unchanged.
